...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    $planId = $_GET['delete'];
    $stmt = $pdo->prepare("SELECT image_url FROM plans WHERE id = ?");
    $stmt->execute([$planId]);
    $plan = $stmt->fetch(PDO::FETCH_ASSOC);

    $stmt = $pdo->prepare("DELETE FROM plans WHERE id = ?");
    if ($stmt->execute([$planId])) {
        if ($plan && $plan['image_url'] && file_exists($plan['image_url'])) {
            unlink($plan['image_url']);
        }
...
if (isset($_GET['toggle_status']) && is_numeric($_GET['toggle_status'])) {
    $stmt = $pdo->prepare("UPDATE plans SET status = CASE WHEN status = 'active' THEN 'inactive' ELSE 'active' END WHERE id = ?");
...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        ...
        <p class="text-sm text-brand-muted mt-1">View and manage all plan entries in the system.</p>
    </div>
    <div class="flex items-center space-x-3">
        ...
        <a href="plan-add" class="inline-flex items-center justify-center px-4 py-2.5 bg-brand-primary text-white text-xs font-bold rounded-xl hover:opacity-90 active:scale-[0.98] transition-all space-x-1.5 shadow-glow">
...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 dark:bg-neutral-900 dark:border-neutral-700">
    <div class="flex items-center justify-between mb-6">
        ...
        <span class="text-xs text-brand-muted font-semibold">Total plans: <?php echo count($plans); ?></span>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-brand-border/40 text-sm">
                ...
                <?php else: ?>
                    ...
                    <tr>
                        <td class="py-3.5 font-mono text-xs text-brand-muted">#<?php echo $plan['id']; ?></td>
                        <td class="py-3.5 font-semibold text-brand-text">
                            <?php echo htmlspecialchars($plan['name']); ?>
                            <div class="md:hidden mt-1">
                                <p class="text-[10px] text-brand-muted font-normal leading-relaxed"><?php echo htmlspecialchars($plan['description'] ?: 'No description'); ?></p>
                            </div>
                        </td>
                        <td class="py-3.5 hidden md:table-cell text-brand-muted text-xs leading-relaxed max-w-xs truncate"><?php echo htmlspecialchars($plan['description'] ?: 'No description provided'); ?></td>
                        <td class="py-3.5">
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase <?php echo $plan['status'] === 'active' ? 'bg-brand-success/10 text-brand-success' : 'bg-brand-muted/20 text-brand-muted'; ?>">
                                <?php echo ucfirst($plan['status']); ?>
                            </span>
                        </td>
                        <td class="py-3.5 text-right">
                            <div class="inline-flex space-x-2">
                                <a href="plan-edit.php?id=<?php echo $plan['id']; ?>" 
                                   title="Edit plan"
                                ...
                                <a href="?toggle_status=<?php echo $plan['id']; ?>" 
                                   title="<?php echo $plan['status'] === 'active' ? 'Pause plan' : 'Activate plan'; ?>"
                                   ...
                                   onclick="return confirm('Change status of this plan?')">
                                    <i class="fa-solid <?php echo $plan['status'] === 'active' ? 'fa-pause text-brand-warning' : 'fa-play text-brand-success'; ?>"></i>
                                </a>

...
// Global Exception Handler for beautiful error pages on ANY fatal error (like missing tables)
set_exception_handler(function($e) {
    error_log("[MultiPanelX Error] " . $e->getMessage());
    http_response_code(500);
    $err = htmlspecialchars($e->getMessage());
    die('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>System Error</title>'
...
function getDBConnection()
{
    static $pdo = null;
    if ($pdo === null) {
        // Will throw PDOException on failure which is caught by the global handler above
